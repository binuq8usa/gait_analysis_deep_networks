...
function crbm = trainCRBM_gpu(X_cpu,opts)
...
for ii = 1:1:crbm.num_layers
    ...
    crbm.rbm{ii}.w = gpuArray(single(0.01*randn(num_hid,num_dims)));
    crbm.rbm{ii}.bi = gpuArray(single(0.01 * randn(num_dims,1)));
    crbm.rbm{ii}.bj = gpuArray(single(-1 + 0.01 * randn(num_hid,1)));
    ...
    crbm.rbm{ii}.A = gpuArray(single(0.01 * randn(num_dims,num_dims,nt)));
    ...
    % layer
    crbm.rbm{ii}.B = gpuArray(single(0.01 * randn(num_hid,num_dims,nt)));
    
    % Initializing weight updates 
    crbm.rbm{ii}.w_update = gpuArray(single(zeros(size(crbm.rbm{ii}.w))));
    crbm.rbm{ii}.bi_update = gpuArray(single(zeros(size(crbm.rbm{ii}.bi))));
    crbm.rbm{ii}.bj_update = gpuArray(single(zeros(size(crbm.rbm{ii}.bj))));
    crbm.rbm{ii}.A_update = gpuArray(single(zeros(size(crbm.rbm{ii}.A))));
    crbm.rbm{ii}.B_update = gpuArray(single(zeros(size(crbm.rbm{ii}.B))));
...
for jj = 1:1:length(crbm.seq_lengths) % iterate through each sequence
    if jj==1 % first sequence
        batchdataindex = gpuArray(single(opts.n{1}+1:crbm.seq_lengths(jj)));
    else
        batchdataindex = [batchdataindex batchdataindex(end)+opts.n{1}+1:batchdataindex(end)+gpuArray(single(crbm.seq_lengths(jj)))];
    end
end
...
for ii = 1:1:crbm.num_layers
    ...
    crbm.rbm{ii} = crbm_train_layer_gpu(crbm.rbm{ii},X_next,minibatch,flag_gaussian);
