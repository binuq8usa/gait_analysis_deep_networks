...
function rbm = crbm_train_layer_gpu(rbm,batchdata,batchdata_inds,flag_gaussian)
...
for epoch = 1:1:rbm.num_epochs
    ...
    for b_num = 1:1:num_batches
        ...
        data = gpuArray(single(zeros(num_frames,num_dims,nt+1)));
        ...
        bi_star = gpuArray(single(zeros(num_dims,num_frames)));
        for hh=1:1:nt
        ...
        bj_star = gpuArray(single(zeros(num_hid,num_frames)));
        for hh = 1:1:nt
        ...
        if(~flag_gaussian)
            hid_states = gpuArray(single(prob_Hj_V_pos' > rand(num_frames,num_hid)));
        else
            hid_states = gpuArray(single(prob_Hj_V_pos' > rand(num_frames,num_hid)));
        end
