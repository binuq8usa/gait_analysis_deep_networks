function fn = func_ap_nn_train(fn,X,Y,opts,flag_seq)
...
for k = 1:1:opts.numepochs
    ...
    for tt = 1:1:floor(numbatches/num_of_seqs_batch)
        ...
        if(k < 6)
            ...
            [~,y_in_data] = nn_ff(fn,data);
            
            % get the input data for the last layer
            [VV, fX] = minimize(VV,'congugateGradientRegularNN',max_iter,Dim,y_in_data,y_data);
            ...
            [VV, fX] = minimize(VV,'congugateGradientRegularNN',max_iter,Dim,data,y_data);
