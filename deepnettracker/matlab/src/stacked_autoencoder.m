...
X_next = X;
fprintf('\nTraining the %d RBM\n',1);
st.rbm{1} = rbmtrain_seq_gpu(st.rbm{1},X_next,opts,flag_seq);
...
for u = 2:1:num_rbms-1
    ...
    st.rbm{u} = rbmtrain_seq_gpu(st.rbm{u},X_next,opts,flag_seq); % TODO : Write up the code for rbmtrain_seq
end
...
fprintf('\nTraining the %d RBM\n',num_rbms);
st.rbm{num_rbms} = rbmtrain_seq_lin_gpu(st.rbm{num_rbms},X_next,opts,flag_seq);
...
opts.numepochs = opts.numepochs * 3;
fprintf('\nPreTraining complete\n');
fprintf('Training the stacked auto-encoder as a neural network\n');
st = sae_nn_train_gpu(st,X,opts,flag_seq);
