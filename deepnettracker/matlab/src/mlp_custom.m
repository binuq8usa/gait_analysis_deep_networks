function [M] = mlp_custom(M, patches, targets, valid_patches, valid_targets, valid_portion, use_cvp)
...
if use_cvp 
    if M.output.binary
        ...
        cvp = crossvalind('Kfold', n_samples,n_minibatches);
    end
end
    ...
    for mb=1:n_minibatches
        ...
        if early_stop
            ...
            if M.output.binary
                ...
                rerr = mean(sum((valid_targets - vr).^2,2));
            end
            if use_gpu > 0
                rerr = gather(rerr);
            end
