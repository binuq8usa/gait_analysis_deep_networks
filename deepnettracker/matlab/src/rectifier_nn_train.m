...
function st = rectifier_nn_train(X,Y,opts,flag_seq)
...
X_next = X;
fprintf('\nTraining the %d RBM\n',1);
st.rbm{1} = rbmtrain_seq_gpu(st.rbm{1},X_next,opts,0);
...
for u = 2:1:num_rbms
    ...
    st.rbm{u} = rbmtrain_seq_gpu(st.rbm{u},X_next,opts,0); % TODO : Write up the code for rbmtrain_seq
end
...
st = deep_nn_train(st,X,Y,opts,flag_seq);
