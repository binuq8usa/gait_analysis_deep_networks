...
function [gen_X,gen_X_hidden1,gen_X_hidden2] = testCRBM_gpu(crbm,num_frames_gen,X_initialdata_cpu,start_fr)

% num_frames_gen : number of frames to generate
X_initialdata = gpuArray(X_initialdata_cpu);
...
visible = gpuArray(single(zeros(num_frames_gen,num_dims)));
...
data = gpuArray(single(zeros(num_cases,num_dims,n1 + 1)));
...
% connections
bj_star = gpuArray(single(zeros(num_hid1,num_cases)));
for hh = 1:1:n1
...
hidden1 = gpuArray(single(ones(num_frames_gen,num_hid1)));
...
if(max_clamped+1 >= num_frames_gen)
    visible = gpuArray(single(zeros(num_frames_gen,num_dims)));
    hidden1 = gpuArray(ones(num_frames_gen,num_hid1));
    hidden2 = gpuArray(ones(num_frames_gen,num_hid2));
    gen_X = visible;
...
for tt = max_clamped+1:num_frames_gen
    ...
    for gg = 1:numGibbs
        ...
        hidden2(tt,:) = gpuArray(single(hposteriors' > rand(1,num_hid2)));
...
for tt = max_clamped+1:num_frames_gen
    ...
    bi_star = gpuArray(single(zeros(num_dims,1)));
    for hh = 1:1:n1
...
gen_X = gather(visible);
gen_X_hidden2 = gather(hidden2);
gen_X_hidden1 = gather(hidden1);
