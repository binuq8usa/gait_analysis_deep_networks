...
for ii=1:1:length(data_angles_seq_train)
    ...
    % compute the shape of trajectory
    trajDescriptor = ComputeShapeOfTrajectory(kinematicModelVectorTrainEst,win_size);
    P_desc_deepNetwork_Proposed_prob_Overall = [P_desc_deepNetwork_Proposed_prob_Overall ; trajDescriptor]; 
    P_desc_deepNetwork_Proposed_prob_Overall_true = [P_desc_deepNetwork_Proposed_prob_Overall_true ; ComputeShapeOfTrajectory(kinematicModelVectorTrain,win_size)]; 
    class_labels_vest_novest_Overall = [class_labels_vest_novest_Overall; (train_class_labels(1)-1)*ones(size(trajDescriptor,1),1)];
end
...
train_vectors_true = double(P_desc_deepNetwork_Proposed_prob_Overall_true);
...
for ii=1:1:length(data_angles_seq_test)
    ...
    trajDescriptor = ComputeShapeOfTrajectory(kinematicModelVectorTestEst,win_size);
    P_desc_deepNetwork_Proposed_prob_Overall = [P_desc_deepNetwork_Proposed_prob_Overall ; trajDescriptor]; 
    P_desc_deepNetwork_Proposed_prob_Overall_true = [P_desc_deepNetwork_Proposed_prob_Overall_true ; ComputeShapeOfTrajectory(kinematicModelVectorTest,win_size)]; 
    class_labels_vest_novest_Overall = [class_labels_vest_novest_Overall; (test_class_labels(1)-1)*ones(size(trajDescriptor,1),1)];
end

test_vectors = double(P_desc_deepNetwork_Proposed_prob_Overall);
test_vectors_true = double(P_desc_deepNetwork_Proposed_prob_Overall_true);
...
%% Using the nearest neighbhor  classifier
num_neighbors = 10;
[ids_org,dis] = yael_nn(single(train_vectors'),single(test_vectors'),num_neighbors);
...
num_neighbors = 10;
[ids_org,dis] = yael_nn(single(train_vectors_true'),single(test_vectors_true'),num_neighbors);
