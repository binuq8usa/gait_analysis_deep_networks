...
% opts.batch_size
function rbm = rbmtrain_seq_gauss_input_gpu(rbm,X,opts,flag_seq)
...
%     end
    ...
    for l = 1:1:numbatches
        ...
        data_mean = mean(x_batch,1); %%% NEED TO USE THESE TO NORMALIZE TEST DATA FOR SEQUENCE GENERATION
        data_std = std(x_batch,1);
        x_batch_norm = (x_batch - repmat(data_mean,num_images_per_batch,1))./repmat(data_std,num_images_per_batch,1);

        %%%%% POSITIVE PHASE
        v0 = x_batch_norm;
        
        num_hid = size(bh_gpu,1);

        % compute the conditional probability that P(H_j = 1| v) for the
        % per-frame features of each sequence
        prob_Hj_V_pos = 1./(1 + exp( -1 * (v0 * W_gpu' + repmat(bh_gpu',num_images_per_batch,1))));
        ...
        pos_Hj_states = (prob_Hj_V_pos > gpuArray(rand(num_images_per_batch,num_hid)));
        ...
        % v1 should be gaussian
        meanData = 1 * single(pos_Hj_states) * W_gpu;
        v1 = meanData + repmat(bv_gpu',num_images_per_batch,1);
        
        % compute P(H_j = 1|v) for negative phase
        prob_Hj_V_neg = 1./(1 + exp( -1 * (v1 * W_gpu' + repmat(bh_gpu',num_images_per_batch,1))));
