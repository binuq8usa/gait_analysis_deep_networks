function st_c = sae_nn_train_gpu(st_c,X_c,opts,flag_seq)
...
X = single(gpuArray(X_c));
st.sizes = gpuArray(st_c.sizes);
st.backprop_size = gpuArray(st_c.backprop_size);
for ii = 1:1:length(st_c.W)
    st.W{ii} = gpuArray(st_c.W{ii});
end
